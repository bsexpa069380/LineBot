...
def handle_message(event):
    ...
    if re.match("我要找老師",message):
        ...
        flex_message = TextSendMessage(text='請選擇你想加強的科目',
            quick_reply=QuickReply(items=[QuickReplyButton(action=MessageAction(label=subject, text="I am Looking For a {sub} Teacher".format(sub=subject))) for subject in SubjectSet             
            ])
        )
        line_bot_api.reply_message(event.reply_token, flex_message)
